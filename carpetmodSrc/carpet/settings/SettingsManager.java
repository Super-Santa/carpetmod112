...
public class SettingsManager {
    ...
    private static List<TriConsumer<ICommandSender, ParsedRule<?>, String>> observers = new ArrayList<>();
    ...
    public static void addRuleObserver(TriConsumer<ICommandSender, ParsedRule<?>, String> observer) {
        observers.add(observer);
    }

    static void notifyRuleChanged(ICommandSender source, ParsedRule<?> rule, String userTypedValue) {
    ...
    public void disableBooleanFromCategory(String category) {
        for (ParsedRule<?> rule : rules.values()) {
            if (rule.type != boolean.class || !rule.categories.contains(category))
                continue;
            ((ParsedRule<Boolean>) rule).set((ICommandSender) server.getCommandManager(), false, "false");
        }
    }
    ...
    private void loadConfigurationFromConf() {
        for (ParsedRule<?> rule : rules.values()) rule.resetToDefault(server.getCommandSenderEntity());
        ...
        for (String key: conf.getLeft().keySet()) {
            if (rules.get(key).set(server.getCommandSenderEntity(), conf.getLeft().get(key)) != null)
    ...
    public boolean setDefaultRule(ICommandSender source, String settingName, String stringValue) {
    ...
    public boolean removeDefaultRule(ICommandSender source, String settingName) {
        if (locked) return false;
        if (rules.containsKey(settingName)) {
            Pair<Map<String, String>,Boolean> conf = readSettingsFromConf();
